...
depth_path = sys.argv[5] if len(sys.argv) > 5 else os.path.join(ROOT, "out/depth3_final.png")
raw_path = sys.argv[6] if len(sys.argv) > 6 else os.path.join(ROOT, "out/depth3.png")
...
for name, path in [("FINAL", os.path.join(ROOT, "out/final.png")),
                   ("AF export", None)]:
    if path is None:
...
p = os.path.join(ROOT, "out/probe_%d_%d.png") % (cx, cy)
out.save(p)
